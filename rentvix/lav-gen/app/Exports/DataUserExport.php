...
class DataUserExport implements
    FromCollection, WithHeadings, WithMapping, WithEvents, WithCustomStartCell, ShouldAutoSize
{
    ...
    {
        $q = DB::table('mst_users');

        if ($this->search) {
            $s = "%{$this->search}%";
            $q->where(function($qq) use ($s) {
                $cols = DB::getSchemaBuilder()->getColumnListing('mst_users');
                foreach ($cols as $c) $qq->orWhere($c, 'like', $s);
            });
        }

        if ($this->status && DB::getSchemaBuilder()->hasColumn('mst_users', $this->statusColumn())) {
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function(AfterSheet $e) {
                ...
                $s->setCellValue('A2', 'Data User - Export');
                ...
                $total = DB::table('mst_users')->count();
                $s->setCellValue("A".($sumTitle+1), 'Total Data User');
                $s->setCellValue("B".($sumTitle+1), $total);

                $statusCol = $this->statusColumn();
                if ($statusCol && \Schema::hasColumn('mst_users', $statusCol)) {
                    $rows = DB::table('mst_users')
