...
export function DataKendaraanFormFields({
    formData,
    setFormData,
    mode,
}: {
...
<div className="space-y-1">
  <Label htmlFor="tahun" className="text-foreground">Tahun*</Label>
  <Input
    type="number"
    id="tahun"
...
<div className="space-y-1">
  ...
  >
    <SelectTrigger>
      <SelectValue placeholder="e.g., active" />
    </SelectTrigger>
    <SelectContent>
...
<ImageUpload
  title="Foto Belakang"
  description="Upload Foto Belakang"
  image={formData.foto_belakang_preview || formData.foto_belakang_url}
  // returnType default "file" → val akan berupa File
  onImageChange={(val) => {
    if (val instanceof File) {
      // Simpan File + preview (match pola stub lama Anda)
      ...
      setFormData((prev) => ({
        ...prev,
        "foto_belakang": val,
        "foto_belakang_preview": preview,
        // reset url kalau sebelumnya pakai url
        "foto_belakang_url": undefined,
      }));
    } else {
      // Kalau suatu saat Anda ingin kirim URL string (mis. hasil upload)
      setFormData((prev) => ({
        ...
        "foto_belakang_preview": undefined,
        "foto_belakang_url": val,
      }));
    }
  }}
/>
