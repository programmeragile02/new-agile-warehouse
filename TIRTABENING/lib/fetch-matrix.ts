...
function buildLocalUrl(path: string, params?: Record<string, string>) {
  ...
  const abs = base ? new URL(path, base.endsWith("/") ? base : base + "/") : new URL(path, "http://localhost/");
  if (params) {
  ...
  return base ? abs.toString() : abs.toString().replace(/^https?:\/\/localhost\/?/, "/");
}
 ...
 */
export async function fetchMatrixInternal(include = "menus,features"): Promise<MatrixPayload> {
  const product = getProductCode();
  const offering = getOffering();

  const url = buildLocalUrl(
    `/api/public/catalog/offerings/${encodeURIComponent(product)}/${encodeURIComponent(offering)}/matrix`,
    { include }
  );
  ...
  try {
    ...
    const cookieHeader = (() => {
      try { return cookies().toString(); } catch { return ""; }
    })();
    ...
    const menus: MatrixMenu[] = Array.isArray(data?.menus) ? data.menus : [];
    const features: MatrixFeature[] = Array.isArray(data?.features) ? data.features : [];
