...
export async function resolveTenant(companyId: string, productCode: string): Promise<TenantInfo | null> {
  const url = `${apiBase()}/tenants/resolve?company_id=${encodeURIComponent(companyId)}&product_code=${encodeURIComponent(productCode)}`;
  const res = await fetch(url, { headers: { "X-API-KEY": apiKey(), Accept:"application/json" }, cache: "no-store" });
