...
export function ScheduleSettingsForm() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            toast({
                title: "Berhasil",
                description: "Pengaturan jadwal disimpan",
            });
        } catch (err: any) {
            toast({
                title: "Gagal",
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-4">
                <div>
                    <Label htmlFor="tanggal-catat">
                        Hari Pencatatan (1–31)
                    </Label>
                    <Input
                        id="tanggal-catat"
                        aria-label="Masukkan tanggal pencatatan"
                        type="number"
                        min={1}
                        max={31}
                    ...
                    />
                    <p className="text-xs text-muted-foreground mt-1">
                        Sistem akan otomatis menyesuaikan jika bulan tidak punya
                        tanggal tsb (mis. 30/31 → jatuh ke 28/29 di Februari).
                    </p>
                </div>
            </div>
