    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                ...
                const r = await fetch("/api/tagihan/unpaid", {
                    cache: "no-store",
                });
                const data = await r.json();
                ...
                toast({
                    ...
                    description:
                        e?.message || "Tidak dapat mengambil daftar tagihan",
                    variant: "destructive",
                });
            } finally {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (!formData.buktiFile)
                throw new Error("Wajib upload bukti pembayaran");
            ...
            setTagihanList((prev) =>
                prev.filter((t) => t.id !== formData.tagihanId)
            );
        } catch (e: any) {
            toast({
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-2">
                    ...
                    {meRole === "WARGA" && (
                        <p className="text-xs text-muted-foreground">
                            Daftar di atas otomatis hanya menampilkan tagihan
                            Anda.
                        </p>
                    )}
                </div>
                    ...
                    <Input
                        value={
                            formData.totalTagihan
                                ? `Rp ${formData.totalTagihan.toLocaleString(
                                      "id-ID"
                                  )}`
                                : ""
                        }
                        ...
                        <div className="flex-1">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                                <div>
                                    ...
                                    <p className="font-bold text-yellow-800">
                                        {formData.denda > 0
                                            ? `Rp ${formData.denda.toLocaleString(
                                                  "id-ID"
                                              )}`
                                            : "Tidak ada denda"}
                                    </p>
                                ...
                                <div>
                                    ...
                                    <p className="font-bold text-yellow-800">
                                        Rp{" "}
                                        {formData.totalDenganDenda.toLocaleString(
                                            "id-ID"
                                        )}
                                    </p>
                                </div>
                                <div>
                                    ...
                                    <p className="text-xs text-yellow-600">
                                        Bulan ke-1: Rp{" "}
                                        {tarif.dendaBulanPertama.toLocaleString(
                                            "id-ID"
                                        )}
                                        <br />
                                        Bulan ke-2+: Rp{" "}
                                        {tarif.dendaBulanKedua.toLocaleString(
                                            "id-ID"
                                        )}
                                    </p>
                                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-2">
                    ...
                    <Input
                        ...
                        value={formData.nominalBayar}
                        onChange={(e) =>
                            handleInput("nominalBayar", e.target.value)
                        }
                        className="h-12 text-base"
                        required
                        min="0"
