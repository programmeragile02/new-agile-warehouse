...
// CustomerList.tsx (atau file komponen list kamu)
"use client";
...
/* ===================== Global invalidate wiring ===================== */
declare global {
    interface Window {
        __tbPelangganChanged?: () => void; // panggil ini setelah POST/PUT/DELETE
    }
}
...
export function CustomerList() {
    ...
    // matcher untuk invalidasi semua varian key /api/pelanggan...
    const invalidateAllPelanggan = useCallback(
        async () =>
            mutate(
                (key) =>
    ...
    // siapkan channel & event listeners sekali
    useEffect(() => {
        // expose helper ke window (bisa dipanggil dari mana pun setelah create/update/delete)
        window.__tbPelangganChanged = () => {
            ...
            // fallback via localStorage ping (memicu 'storage' di tab lain)
            try {
        ...
        try {
            bc = new BroadcastChannel("tb:pelanggan");
            bc.onmessage = (e) => {
                if (e?.data?.type === "changed") invalidateAllPelanggan();
            };
        } catch {}

        // listener CustomEvent (same tab)
        const onCustom = () => invalidateAllPelanggan();
        window.addEventListener("tb:pelanggan:changed", onCustom);

        // listener storage (cross tab fallback)
        const onStorage = (e: StorageEvent) => {
            if (e.key === "tb:pelanggan:ping") invalidateAllPelanggan();
        };
        window.addEventListener("storage", onStorage);

        return () => {
            window.removeEventListener("tb:pelanggan:changed", onCustom);
            window.removeEventListener("storage", onStorage);
            try {
                bc?.close();
            } catch {}
        };
    }, [invalidateAllPelanggan]);
    ...
    const {
        ...
        error,
        isLoading,
        mutate: mutateThisList,
    } = useSWR<ApiResp>(listKey, fetcher, {
        // supaya responsif ketika diminta revalidate
    ...
    const confirmDelete = async () => {
        ...
        try {
            ...
            // invalidate semua varian list
            await invalidateAllPelanggan();
        } catch (e) {
            const msg = e instanceof Error ? e.message : "Terjadi kesalahan.";
            toastApi({
                title: "Gagal Menghapus",
    ...
    const onDragEnd = async (event: DragEndEvent) => {
        ...
        // optimistic UI (hanya untuk subset zona)
        await mutate(
            (key) => key === listKey,
            (prev?: ApiResp) => {
                if (!prev?.ok || !Array.isArray(prev.items)) return prev;
        ...
        try {
            ...
            await invalidateAllPelanggan(); // sinkron ulang semua list
        } catch (err) {
            await mutateThisList(); // fallback revalidate list sekarang
            toast({
                title: "Gagal reorder",
                description:
                    err instanceof Error ? err.message : "Terjadi kesalahan",
