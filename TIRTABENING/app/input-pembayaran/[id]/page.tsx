...
export default function InputPembayaranPage() {
    ...
    async function refetchTagihan(tagihanId: string) {
        try {
            const r = await fetch(`/api/tagihan/${tagihanId}`, {
                cache: "no-store",
            });
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                const r = await fetch(`/api/tagihan/${id}`, {
                    cache: "no-store",
                });
                const data = await r.json();
                if (!r.ok || !data?.ok)
    ...
    const refetchPembayaran = async (tagihanId: string) => {
        ...
        try {
            const r = await fetch(
                `/api/pembayaran/by-tagihan?tagihanId=${encodeURIComponent(
                    tagihanId
                )}`,
                { cache: "no-store" }
            );
            ...
            if (r.ok && d?.ok) {
                ...
                if (d.pembayaran?.metode)
                    setMetode(d.pembayaran.metode as Metode);
                if (
                    !revisiMode &&
                    typeof d.pembayaran?.keterangan === "string"
                ) {
    ...
    const performUpload = async () => {
        try {
            ...
            setT((prev) =>
                prev
                    ? ({ ...prev, statusBayar: "PAID" } as TagihanDetail)
                    : prev
            );
            removeProof();
    ...
    async function handleConfirmApprove() {
        try {
            ...
            setT((prev) =>
                prev
                    ? ({ ...prev, statusVerif: "VERIFIED" } as TagihanDetail)
                    : prev
            );
            setRevisiMode(false);
            router.replace("/tagihan-pembayaran");
    ...
    async function handleStartRevisi() {
        ...
        try {
            ...
            if (!r.ok || !j?.ok)
                throw new Error(
                    j?.message || "Gagal mengubah status verifikasi"
                );
            setT((prev) =>
                prev
                    ? ({ ...prev, statusVerif: "UNVERIFIED" } as TagihanDetail)
                    : prev
            );
            setRevisiMode(true);
            toast({
                ...
                description:
                    "Silakan perbaiki data pembayaran lalu simpan & approve.",
            });
        } catch (e: any) {
            toast({
                title: "Gagal",
                            ...
                            yang tersimpan tetap angka murni.
                                                ...
                                                {!hideProofForViewer ? (
                                                    ...
                                                    <div className="rounded-md p-3 bg-muted/30 text-xs text-muted-foreground">
                                                        Pembayaran <b>TUNAI</b>{" "}
                                                        — bukti pembayaran tidak
                                                        ditampilkan.
                                                    </div>
                                                )}
                                                <div className="mt-4">
