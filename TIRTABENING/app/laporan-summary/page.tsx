...
export default function DashboardLaporanPage() {
    ...
    useEffect(() => {
        // Muat daftar tahun & data pertama kali
        refreshAvailableYears();
        getDataByYear(selectedYear);
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, []);

    const revMax = Math.max(0, ...revenueData.map((d) => d.amount || 0));
    const expMax = Math.max(
        0,
        ...expenseData.map((d) => (d.operasional || 0) + (d.lainnya || 0))
    );
        ...
        <div className="min-h-screen">
            <AuthGuard>
                <AppShell>
                    ...
                    <div className="container mx-auto px-4 space-y-6">
                        {/* Filter Tahun (otomatis dari data) */}
                        <GlassCard className="p-4">
                            <div className="flex items-center gap-4">
                                <label className="text-sm font-medium text-foreground">
                                    Tahun:
                            ...
                            <GlassCard className="p-6">
                                ...
                                <ResponsiveContainer width="100%" height={300}>
                                    <LineChart data={waterUsageData}>
                                        ...
                                        {/* Garis blok: render sesuai jumlah zona yang ada */}
                                        {(() => {
                                            const keys = [
                                                "blokA",
                                                "blokB",
