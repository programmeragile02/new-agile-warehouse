...
export default function HutangPembayaranPage() {
    ...
    const { data, isLoading, mutate } = useSWR<{
        ...
        giver
            ? `/api/hutang-pembayaran?giver=${encodeURIComponent(giver)}`
            : null,
        fetcher,
        { revalidateOnFocus: false }
    );
    ...
    const savePayment = async () => {
        ...
        try {
            ...
            toast({
                title: "Tersimpan",
                description: `Total bayar ${toIDR(
                    j?.payment?.total || totalBayar
                )}`,
            });
        } catch (e: any) {
            toast({
