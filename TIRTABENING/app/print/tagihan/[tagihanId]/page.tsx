...
//     const perusahaan = setting.namaPerusahaan || "Nata Banyu";
...
export default async function Page({ params, searchParams }: PageProps) {
    ...
    // ===== ambil tagihan + relasi catatMeter =====
    const tagihan = await prisma.tagihan.findUnique({
        where: { id: tagihanId },
        select: {
            id: true,
    ...
    // ----------------------
    // Build logo src carefully:
    // - prefer NEXT_PUBLIC_APP_URL (set in env)
    // - if setting.logoUrl is full URL use it, else join origin+path
    // ----------------------
    const origin = (
    ...
    const rawLogo = (setting as any).logoUrl || ""; // expects like "/uploads/xxx.png" or full url
    const logoSrc =
        rawLogo.startsWith("http://") || rawLogo.startsWith("https://")
            ? rawLogo
            : origin
            ? `${origin}${rawLogo.startsWith("/") ? rawLogo : "/" + rawLogo}`
            : rawLogo || "/logo.png";
    ...
    return (
      ...
      `}</style>
            ...
            <div className="max-w-[380px] mx-auto">
                ...
                <div className="headerBar">
                    <div className="w-14 h-14 rounded-md overflow-hidden flex items-center justify-center">
                        {/* Use plain <img> with absolute URL so Puppeteer can load it reliably */}
                        <img
                            src={logoSrc}
                            alt="Logo Perusahaan"
                            style={{
