...
import { useToast } from "@/hooks/use-toast";
import { Textarea } from "@/components/ui/textarea";
import { LockRowModal } from "@/components/lock-row-blok";
import { format } from "date-fns";
import { id } from "date-fns/locale";

// ⬇️ NEW: Tooltip & Dialog (untuk mobile)
import {
    Tooltip,
    TooltipContent,
    TooltipProvider,
...
/* ===== NEW: helper mobile + komponen InfoTip ===== */
        ...
        <AuthGuard requiredRole="PETUGAS">
            <AppShell>
                <AppHeader
                    ...
                    titleExtra={
                        <InfoTip
                            ariaLabel="Apa itu Catat Meter Blok?"
                            open={openTip}
                            onOpenChange={setOpenTip}
                        >
                            <b>Catat Meter Blok</b> dipakai untuk mencatat angka
                            meter blok per periode. Saat baris dikunci atau
                            status periode <b>FINAL</b>, nilainya tidak bisa
                            diubah.
                        </InfoTip>
                    }
                />
...
function CatatBlokContent() {
    ...
    useEffect(() => {
        (async () => {
            const res = await fetch("/api/tandon/list", {
                cache: "no-store",
            }).catch(() => null);
            if (!res?.ok) return setTandons([]);
            const js = await res.json();
            const arr = Array.isArray(js?.items)
                ? (js.items as TandonOpt[])
                : [];
            setTandons(arr);
            if (!tandonId && arr.length) setTandonId(arr[0].id);
        })();
    ...
    const isAllLocked =
        filtered.length > 0 && filtered.every((r) => !!r.locked);
    ...
    async function saveRow(row: Row, endOverride?: number) {
        ...
        try {
            ...
            const js = await res.json();
            if (!res.ok || !js?.ok)
                throw new Error(js?.message || "Gagal simpan");
            if (key) await mutate(key);
            setBufEnd(({ [row.id]: _drop, ...rest }) => rest);
            toast({ title: "Tersimpan", description: `Blok ${row.nama}` });
        } catch (e: any) {
    ...
    async function finalizeRow(row: Row) {
        try {
            ...
            const js = await res.json();
            if (!res.ok || !js?.ok)
                throw new Error(js?.message || "Gagal finalize");
            if (key) await mutate(key);
            toast({
                title: "Terkunci",
                description: `Blok ${row.nama} dikunci & carry ke bulan depan`,
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-4 mt-6">
                    ...
                    <div className="md:col-span-2 lg:col-span-3">
                        ...
                        {data?.ok && (
                            <div className="mt-2 flex items-center justify-between">
                                <p className="text-xs text-muted-foreground">
                                    Progress: {data.progress.selesai}/
                                    {data.progress.total} (
                                    {Math.round(progressPct)}%)
                                </p>
                            </div>
                ...
                <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-2 mb-4">
                    <div>
                        ...
                        {data?.ok && (
                            <p className="text-sm text-muted-foreground">
                                Progress: {data.progress.selesai}/
                                {data.progress.total} ({Math.round(progressPct)}
                                %)
                            </p>
                        )}
                    </div>
                ...
                <GlassCard className="p-4">
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                        ...
                        <div className="rounded-xl border bg-green-50/50 px-6 py-6 text-center">
                            ...
                            <div className="mt-1 text-sm text-green-800/80">
                                Progress
                            </div>
                        </div>
                    </div>
