...
//             .filter(
//                 (m) =>
//                     m.periodeCatat?.startsWith(String(thisYear)) && m.usage > 0
//             )
//             .map((m) => m.usage);
//         return used.length
...
//             .filter(
//                 (m) =>
//                     m.periodeCatat?.startsWith(String(thisYear)) &&
//                     m.status === "paid"
//             )
//             .reduce((s, m) => s + (m.bill || 0), 0);
//     }, [yearly]);

//     function formatPeriodeCatat(p: string) {
//         // support "YYYY-MM" or fallback
//         if (!p) return "-";
//         const parts = p.split("-");
//         if (parts.length >= 2) {
...
//                                                 <p className="text-2xl font-bold text-red-600">
//                                                     Rp{" "}
//                                                     {current.totalTagihan.toLocaleString(
//                                                         "id-ID"
//                                                     )}
//                                                 </p>
//                                                 <p className="text-sm text-muted-foreground">
...
export default function WargaDashboardPage() {
    const { setIssues, getIssuesByCustomer, upsertIssue } =
        useWaterIssuesStore();
    const [customerIssues, setCustomerIssues] = useState<any[]>([]);
    ...
    // Load kendala warga dari server (filter by customerId)
    useEffect(() => {
        if (!resident?.customerId) {
            setCustomerIssues([]);
            return;
        ...
        (async () => {
            try {
                const res = await fetch(
                    `/api/kendala?customerCode=${encodeURIComponent(
                        resident.customerId
                    )}`,
                    { cache: "no-store" }
                );
                ...
                if (!cancelled && Array.isArray(data.items)) {
                    // timpa data kendala di store dengan hasil dari server
                    setIssues(data.items);
                    setCustomerIssues(getIssuesByCustomer(resident.customerId));
                }
            } catch (e) {
                console.error("Gagal memuat kendala warga:", e);
            }
        })();
    ...
    }, [resident?.customerId, setIssues, getIssuesByCustomer]);
        ...
        try {
            const res = await fetch("/api/kendala", {
                ...
                body: JSON.stringify({
                    ...
                    priority: "medium",
                    // backend pakai kode pelanggan
                    customerCode: resident.customerId,
                }),
            });

            const data = await res.json();
            if (!res.ok)
                throw new Error(data?.error || "Gagal mengirim kendala");

            if (data.item) {
                // sinkronkan ke store
                upsertIssue(data.item);
            }
            ...
            toast({
                title: "Terima kasih",
                description:
                    "Kendala / saran kamu sudah kami terima dan akan ditindaklanjuti.",
            });
        } catch (e: any) {
            console.error(e);
            toast({
                title: "Gagal mengirim kendala",
                description:
                    e?.message || "Silakan coba lagi beberapa saat lagi.",
                variant: "destructive",
            });
        } finally {
                        ...
                        {!error && resident && (
                            <GlassCard className="p-6">
                                <div className="flex items-center gap-3 mb-4">
                                    ...
                                    <div>
                                        <h2 className="text-xl font-semibold text-foreground">
                                            Laporkan Kendala / Saran
                                        </h2>
                                        <p className="text-sm text-muted-foreground">
                                            Sampaikan keluhan atau masukan
                                            terkait pelayanan air di lingkungan
                                            Anda.
                                        </p>
                                    </div>
                                </div>
                                ...
                                <div className="space-y-3">
                                    <div>
                                        ...
                                        <Input
                                            placeholder="Contoh: Air keruh sejak kemarin malam"
                                            value={issueTitle}
                                            onChange={(e) =>
                                    ...
                                    <div>
                                        ...
                                        <Textarea
                                            placeholder="Silakan jelaskan waktu kejadian, lokasi, dan detail lain yang perlu diketahui oleh pengelola/admin..."
                                            value={issueDesc}
                                            onChange={(e) =>
