...
// helper untuk mendapatkan YYYY-MM bulan berjalan
function currentYm() {
    const d = new Date();
...
export default function RiwayatPetugasPage() {
    ...
    async function loadData() {
        ...
        try {
            ...
            if (j?.ok) {
                ...
                // set periods jika disediakan
                if (Array.isArray(j.periods)) {
                    setPeriods(j.periods);

                    // otomatis pilih periode bulan berjalan jika tersedia dan user belum memilih
                    const cur = currentYm();
                    if (!periode && j.periods.includes(cur)) {
                        // setPeriode akan memicu reload karena periode ada di deps useEffect
                        setPeriode(cur);
                        // reset page ke 1 agar konsisten
                        setPage(1);
                    }
                    // jika user sudah memilih periode yang sekarang tidak ada dalam j.periods, biarkan kosong (atau reset)
                    if (periode && !j.periods.includes(periode)) {
                        setPeriode("");
                    }
                }
    ...
    // helper render for periode options: if `periods` available use it, otherwise fallback to hardcoded.
    const renderPeriodeOptions = () => {
        ...
        // fallback (preserve previous UI content if API hasn't returned periods yet)
        return (
            <>
                            ...
                            <div className="space-y-1 flex-1">
                                ...
                                <div className="relative">
                                    ...
                                    <Input
                                        ...
                                        placeholder="Cari pelanggan/blok/kendala…"
                                        value={q}
                                        onChange={(e) => {
                                            setQ(e.target.value);
                                            setPage(1);
