...
//                 const fromApi: typeof manual = (data.items ?? []).map(
//                     (it: any) => ({
//                         id: it.id,
//                         issue: it.issue,
...
export default function KendalaPage() {
    ...
    const { issues, setIssues, upsertIssue } = useWaterIssuesStore();
    const { toast } = useToast();

    // === HYDRATE dari API (semua dari DB + catat meter) ===
    useEffect(() => {
        let cancelled = false;
        ...
        (async () => {
            try {
                ...
                if (!cancelled && Array.isArray(data.items)) {
                    // sekarang store versi baru: replace penuh dari API
    ...
    // PATCH → tandai selesai + simpan solusi ke DB
    const handleSolveIssue = async (issueId: string) => {
        if (!solution.trim()) return;

        try {
            const res = await fetch(`/api/kendala/${issueId}`, {
                method: "PATCH",
            ...
            if (!res.ok) {
                throw new Error(data?.error || "Gagal memperbarui kendala");
            }
            ...
            toast({
                title: "Gagal",
                description:
                    e?.message || "Tidak bisa menandai kendala sebagai selesai",
                variant: "destructive",
            });
        }
    };

    // POST → tambah kendala baru (admin) ke DB
        ...
        try {
            const res = await fetch("/api/kendala", {
                ...
                body: JSON.stringify({
                    ...
                    // admin tidak kirim customerCode → pure manual
                }),
            });

            const data = await res.json().catch(() => ({}));
            if (!res.ok) {
                throw new Error(data?.error || "Gagal menyimpan kendala");
            }
            ...
            toast({
                title: "Kendala Ditambahkan",
                description: "Kendala baru berhasil disimpan ke database",
            });
        } catch (e: any) {
            console.error(e);
            toast({
                title: "Gagal",
                description: e?.message || "Gagal menyimpan kendala",
                variant: "destructive",
            });
        }
    };
    ...
    const getSourceLabel = (issue: (typeof issues)[number]) => {
        if (
            issue.source === "manual_report" &&
            issue.customerId &&
            issue.customerId !== ""
        ) {
            return "Laporan Warga";
        }
        if (issue.source === "manual_report") return "Laporan Manual";
        if (issue.source === "meter_reading") return "Dari Catat Meter";
        if (issue.source === "meter_reading_blok")
            return "Dari Catat Meter (Blok)";
        return "Laporan";
    };
