...
import { useToast } from "@/hooks/use-toast";
import { TandonLockModal } from "@/components/lock-row-tandon";
import { format } from "date-fns";
import idLocale from "date-fns/locale/id";
...
export default function CatatTandonPage() {
    ...
    const rowsFilteredByTandon =
        tandonId === "ALL"
            ...
            : rowsAll.filter(
                  (r) =>
                      r.kodeTandon ===
                      tandons.find((t) => t.id === tandonId)?.kode
              );
    const rows = (rowsFilteredByTandon || []).filter(
        (r) =>
            r.nama.toLowerCase().includes(searchTerm.toLowerCase()) ||
    ...
    const saveRow = async (row: Row, endOverride?: number) => {
        ...
        try {
            ...
            toast({
                title: "Tersimpan",
                description: `Catat Meter ${row.nama}`,
            });
        } catch (e: any) {
            toast({
                title: "Gagal simpan",
            ...
            <AppShell>
                <AppHeader
                    ...
                    titleExtra={
                        ...
                        >
                            <b>Catat Meter Tandon</b> dipakai untuk mencatat
                            angka meter tandon per periode. Saat baris dikunci
                    ...
                    <GlassCard className="p-6">
                        ...
                        {periode && (
                            <div className="flex flex-wrap items-center gap-3 px-1">
                                ...
                                <Badge
                                    variant={
                                        isPeriodeFinal ? "default" : "secondary"
                                    }
                                    className={
                                        isPeriodeFinal
                                            ? "bg-green-100 text-green-800 hover:bg-green-100"
                            ...
                            <div className="md:col-span-2 lg:col-span-3">
                                ...
                                {data?.ok && (
                                    <p className="mt-2 text-xs text-muted-foreground">
                                        ...
                                        {data.progress.total} ({progressPct}%)
                                    </p>
                                )}
                            </div>
                        </div>
