...
export default function MasterTandonPage() {
    ...
    function openAdd() {
        if (quotaHabis) {
            toast({
                ...
                description:
                    "Paket sudah mencapai maksimum tandon. Hapus tandon yang tidak dipakai atau upgrade paket.",
                variant: "destructive",
            });
            return;
        }
    ...
    async function handleGenerate() {
        ...
        try {
            ...
            if (!res.ok || !js?.ok)
                throw new Error(js?.message || "Gagal generate");
            setKode(js.kode || "");
        } catch (e: any) {
            toast({
    ...
    async function handleSave() {
        ...
        // client-side guard: jika kuota habis - jangan kirim tambah baru
        if (quotaHabis && !edit) {
            toast({
                title: "Kuota tandon habis",
                description:
        ...
        try {
            ...
            if (!res.ok || !js?.ok) {
                // jika server balikin quota exceeded
                if (js?.code === "QUOTA_EXCEEDED") {
    ...
    async function handleDelete(x: Tandon) {
        ...
        try {
            ...
            const js = await res.json();
            if (!res.ok || !js?.ok)
                throw new Error(js?.message || "Gagal hapus");
    ...
    const tipContent = (
        <div className="text-sm">
            <strong>Tandon</strong> adalah sumber air (bisa sumur, mata air,
            atau penampungan) yang airnya dialirkan ke meteran di tiap{" "}
            <em>blok</em> untuk dicatat pemakaiannya.
        </div>
    );
                ...
                <AppShell>
                    <div className="max-w-6xl mx-auto space-y-6">
                        ...
                        <GlassCard className="p-6">
                            ...
                            {/* Banner kuota (tampil jika kuota habis) */}
                            {quotaHabis && (
                                <div className="mt-4 p-4 border border-amber-300 bg-amber-50 text-amber-900 rounded-md">
                                    <div className="font-medium">
                                        Kuota tandon habis
                                    </div>
                                    <div className="text-sm text-justify leading-relaxed">
                                        Paket kamu sudah mencapai maksimum
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="space-y-2">
                                    ...
                                    <p className="text-xs text-muted-foreground">
                                        Hanya dipakai pada periode pertama.
                                        Periode berikutnya akan memakai meter
