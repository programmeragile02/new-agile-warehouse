...
export default function InventarisPage() {
    ...
    async function onCreatePurchase() {
        ...
        const tanggalDenganJam = composeDateWithTime(
            purchase.tanggal,
            nowHHmm()
        );
        ...
        try {
            const r = await fetch("/api/inventaris/purchase", {
                ...
                body: JSON.stringify({
                    ...purchase,
                    tanggal: tanggalDenganJam,
                }),
            });
            const j = await r.json();
            if (!j.ok)
                throw new Error(j.message || "Gagal menyimpan pembelian");
            setPurchase({
                tanggal: "",
                supplier: "",
                itemId: "",
    ...
    async function postingById(id: string) {
        askConfirm(
            ...
            async () => {
                setConfirmBusy(true);
                const r = await fetch(
                    `/api/inventaris/purchase/${id}/posting`,
                    {
                        method: "POST",
                    }
                );
