    ...
    return (
        ...
        >
            <div className="flex items-center gap-2">
                ...
                {extra ? <div className="ml-1">{extra}</div> : null}
            </div>
            {typeof count === "number" && (
...
/* ====== NEW: Info tip component (Tooltip desktop, Dialog mobile) ====== */
...
function InfoTip({
    ...
    ariaLabel: string;
    children: React.ReactNode;
    open?: boolean; // optional (untuk Dialog terkontrol)
        ...
        return (
            <>
                <button
                    ...
                    className="inline-flex items-center justify-center rounded-full p-2 hover:bg-muted/50"
...
export default function RekonsiliasiPage() {
    ...
    // NEW: control for title tooltip Dialog (mobile)
    const [openTitleTip, setOpenTitleTip] = useState(false);
    // NEW: section tooltips for mobile
    const [openTipTandon, setOpenTipTandon] = useState(false);
    const [openTipBlok, setOpenTipBlok] = useState(false);
    const [openTipRumah, setOpenTipRumah] = useState(false);
    ...
    const {
        ...
        periode
            ? `/api/distribusi/rekonsiliasi/tandon?periode=${periode}`
            : null,
        fetcher
    );
    ...
    const tipTitle = (
        <>
            <strong>Rekonsiliasi</strong> membandingkan total <em>air masuk</em>{" "}
            dari meter tandon dengan <em>air keluar</em>:
            <br />
            • Level Tandon: input tandon vs total yang dialirkan ke blok.
            <br />
            • Level Blok: input blok vs total pemakaian rumah.
            <br />• Level Pelanggan: angka pemakaian per rumah pada periode
            tersebut.
        </>
    );
    const tipTandon = (
        <>
            <strong>Tandon</strong>: membandingkan{" "}
            <em>Air masuk tandon (m³)</em> dengan{" "}
            <em>Total masuk ke blok (m³)</em>. Selisih =
            kebocoran/ketidakcocokan di antara tandon &gt; jaringan blok.
        </>
    );
    const tipBlok = (
        <>
            <strong>Blok</strong>: membandingkan <em>Air ke blok (m³)</em>{" "}
            dengan <em>Total pemakaian rumah (m³)</em>. Selisih = potensi
            kebocoran di distribusi dalam blok atau data meter rumah yang belum
            lengkap.
        </>
    );
    const tipRumah = (
        <>
            <strong>Pelanggan</strong>: daftar pemakaian (m³) tiap rumah untuk
            periode {periodeLabel || "-"} dari meter pelanggan.
        </>
    );
    ...
    return (
        <AuthGuard requiredRole={"ADMIN"}>
            ...
            >
                <AppShell>
                    {/* ===== Header (dengan tooltip di judul) ===== */}
                    <AppHeader
                        title="Rekonsiliasi"
                        titleExtra={
                            <InfoTip
                                ariaLabel="Penjelasan rekonsiliasi"
                                open={openTitleTip}
                    ...
                    <div className="px-0 grid grid-cols-1 md:grid-cols-3 gap-3 pb-4">
                        <GlassCard className="p-4">
                            <div className="flex items-center gap-1">
                                <div className="text-xs text-muted-foreground">
                                    Total air masuk (semua tandon) –{" "}
                                    {periodeLabel || "-"}
                                </div>
                                {/* tip kecil untuk KPI agregat */}
                                <InfoTip
                                    ariaLabel="Info total air masuk"
                                    // tidak perlu control state khusus; biarkan otomatis
                                >
                                    Total kumulatif input meter di semua tandon
                                    pada periode dipilih.
                                </InfoTip>
                            </div>
                        ...
                        <GlassCard className="p-4">
                            <div className="flex items-center gap-1">
                                ...
                                <InfoTip ariaLabel="Info total ke blok">
                                    Total kumulatif yang tercatat mengalir dari
                                    tandon ke seluruh blok pada periode dipilih.
                                </InfoTip>
                            </div>
                            <div className="text-lg font-bold">
                        ...
                        <GlassCard className="p-4 flex items-center justify-between gap-3">
                            <div>
                                <div className="flex items-center gap-1">
                                    ...
                                    <InfoTip ariaLabel="Info selisih total">
                                        Selisih = Air masuk tandon − Air ke
                                        blok. Persentase = Selisih ÷ Air masuk
                                        tandon × 100%.
                                    </InfoTip>
                                </div>
                                <div className="text-lg font-bold">
                    ...
                    <div className="px-0 pb-4">
                        <GlassCard className="p-3 overflow-hidden hidden md:block">
                            <div className="p-3 border-b font-semibold flex items-center gap-2">
                                ...
                                <InfoTip
                                    ariaLabel="Info ringkasan tandon"
                                    open={openTipTandon}
                                    onOpenChange={setOpenTipTandon}
                                >
                                    {tipTandon}
                                </InfoTip>
                            </div>
                    ...
                    <div className="px-0 pb-6 hidden md:block">
                        <GlassCard className="p-3 overflow-hidden">
                            <div className="p-3 border-b font-semibold flex items-center gap-2">
                                Perincian Blok (Air ke blok → Total pemakaian
                                rumah)
                                <InfoTip
                                    ariaLabel="Info perincian blok"
                                    open={openTipBlok}
                                    onOpenChange={setOpenTipBlok}
                                >
                                    {tipBlok}
                                </InfoTip>
                            </div>
                        ...
                        <DialogContent className="w-[92vw] max-w-3xl max-h-[85vh] overflow-y-auto p-3 sm:p-6 rounded-2xl">
                            <DialogHeader>
                                <DialogTitle className="flex items-center gap-2">
                                    Daftar pelanggan di{" "}
                                    {blokSelected?.nama ?? "-"}
                                    {/* Info di header dialog pelanggan */}
                                    <InfoTip
                                        ariaLabel="Info pelanggan"
                                        open={openTipRumah}
                                        onOpenChange={setOpenTipRumah}
                                    >
                                        {tipRumah}
                                    </InfoTip>
                                </DialogTitle>
                            ...
                            <GlassCard className="mt-3 p-3 overflow-hidden">
                                <div className="p-2 border-b text-sm text-muted-foreground flex items-center gap-2">
                                    ...
                                    {rumahFiltered.length} pelanggan
                                </div>
                                <div className="overflow-x-auto">
                                    <table className="w-full text-sm">
                                        <thead className="bg-muted/40">
