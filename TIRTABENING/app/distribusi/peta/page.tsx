...
export default function DistribusiPetaPage() {
    ...
    const [tab, setTab] = useState<"ALL" | "NORMAL" | "ANOMALY" | "ZERO">(
        "ALL"
    );
    ...
    const stat = useMemo(() => {
        ...
        return {
            ...
            coordsMissing:
                all.length -
                all.filter((r) => r.lat != null && r.lng != null).length,
        };
    }, [data]);
    ...
    const tipTitle = (
        <>
            <strong>Peta Pemakaian Air</strong> menampilkan titik rumah
            berdasarkan koordinat. Warna/status:
            <br />• <b>Hijau (Normal)</b> = pemakaian dalam rentang wajar vs
            rata-rata pembanding.
            <br />• <b>Merah (Abnormal)</b> = berbeda signifikan dari rata-rata
            (diukur oleh ambang <em>Sensitivitas</em>).
            <br />• <b>Abu-abu (0 m³)</b> = tidak ada pemakaian pada periode
            ini.
            <br />
            Gunakan filter <em>Periode</em>, <em>Blok</em>,{" "}
            <em>Sensitivitas</em>, dan <em>Periode Pembanding</em> untuk
            menyesuaikan hasil.
        </>
    );
    const tipPeriode = (
        <>
            Periode catat meter yang dianalisis. Jika kosong, sistem memakai
            periode terbaru dari server.
        </>
    );
    const tipThreshold = (
        <>
            <strong>Sensitivitas</strong> mengatur seberapa besar penyimpangan
            dari rata-rata yang dianggap <em>Abnormal</em>.
            <br />
            Ketat (±30%) &rarr; mudah terdeteksi; Longgar (±75%) &rarr; lebih
            toleran.
        </>
    );
    const tipBaseline = (
        <>
            Jumlah bulan yang dipakai untuk menghitung{" "}
            <em>rata-rata pembanding</em>. Jika riwayat kurang dari pilihan,
            sistem memakai yang tersedia.
        </>
    );

    return (
        <PermissionGate
            path="/distribusi/peta"
            action="view"
                ...
                <div className="space-y-4">
                    ...
                    {!!stat.lowHistory && (
                        <GlassCard className="p-3 flex items-start gap-2">
                            ...
                            <div className="text-sm">
                                {stat.lowHistory} pelanggan punya riwayat &le; 1
                                bulan. Hasil deteksi mungkin kurang akurat.
                                Tambahkan data periode sebelumnya agar baseline
                                lebih stabil.
                            </div>
                        </GlassCard>
                    )}
                    {!!stat.coordsMissing && (
                        <GlassCard className="p-3 flex items-start gap-2">
                            <Info className="w-4 h-4 mt-0.5" />
                            <div className="text-sm">
                                {stat.coordsMissing} pelanggan belum memiliki
                                koordinat (lat/lng). Edit pelanggan untuk
                                mengisi Latitude & Longitude agar muncul di
                                peta.
                            </div>
                        </GlassCard>
                    )}
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-5 gap-3">
                            ...
                            <div className="flex flex-col">
                                ...
                                <div className="text-[11px] text-muted-foreground mt-1 flex items-center gap-1">
                                    <Calendar className="w-3 h-3" /> Jika
                                    kosong, otomatis pakai yang terbaru.
                                </div>
                            </div>
                    ...
                    <div className="text-xs text-muted-foreground">
                        ...
                        <b>rata-rata {baselineN} bulan terakhir</b> yang
                        tersedia. Jika jumlah bulan riwayat lebih sedikit dari
                        pilihan, sistem otomatis memakai yang ada. Ambang
                        deteksi: <b>±{Number(threshold) * 100}%</b>.
                    </div>
                </div>
            </AppShell>
        </PermissionGate>
