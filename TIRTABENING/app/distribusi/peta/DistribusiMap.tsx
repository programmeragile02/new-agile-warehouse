...
export default function DistribusiMap({
  ...
  return (
    <div style={{ height, width: "100%" }}>
      <MapContainer
        ref={(m) => {
          mapRef.current = (m as unknown as LMap) || null;
        }}
        center={L.latLng(-6.2, 106.8167)}
        zoom={12}
      ...
      >
        ...
        {points.map((p) => (
          <Marker
            ...
            ref={(mk) => {
              if (mk) markerRefs.current.set(p.id, mk);
            }}
            eventHandlers={{
            ...
            <Popup>
              <div className="space-y-1">
                ...
                <div className="text-sm mt-2">
                  Pemakaian: <b>{p.pemakaianM3} m³</b>
                </div>
                <div className="text-sm">
                  Rata-rata:{" "}
                  {p.baselineAvg != null
                    ? `${p.baselineAvg.toFixed(1)} m³ (${p.baselineCount} bln)`
                    : "-"}
                </div>
                <div className="text-sm">
                  Perubahan:{" "}
                ...
                <div className="mt-2">
                  {p.status === "NORMAL" && (
                    <Badge className="bg-emerald-500 text-white">Normal</Badge>
                  )}
                  {p.status === "ANOMALY" && (
                    <Badge className="bg-red-500 text-white">Abnormal</Badge>
                  )}
                  {p.status === "ZERO" && (
                    <Badge className="bg-gray-400 text-white">0 m³</Badge>
                  )}
                </div>
              </div>
            </Popup>
