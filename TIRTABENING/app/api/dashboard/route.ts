...
function hitungSaldoAkhir(
    ...
    const fallback =
        (totalTagihan || 0) + (tagihanLalu || 0) - (totalBayar || 0);
...
export async function GET(req: Request) {
    ...
    try {
        ...
        for (const t of tagihanTahun) {
            ...
            const net = Math.max(
                (t.totalTagihan || 0) + (t.tagihanLalu || 0),
                0
            ); // <-- incl. carry, no minus
        ...
        for (const p of periods) {
            ...
            const cmRows = await prisma.catatMeter.findMany({
                where: {
                    deletedAt: null,
                    periode: { tahun: p.tahun, bulan: p.bulan },
                },
                select: { pemakaianM3: true },
            });
            const totalM3 = cmRows.reduce(
                (s, r) => s + (r.pemakaianM3 || 0),
                0
            );
            ...
            if ((totalTagihanLalu || 0) > 0) {
                ...
                infoBadge = `Terpakai saldo kredit ${Intl.NumberFormat(
                    "id-ID"
                ).format(Math.abs(totalTagihanLalu))}`;
            }
            if (sisaAkhirLebih > 0) {
        ...
        // Water issues
        const issues = await prisma.kendalaAir.findMany({
            orderBy: [
                { status: "asc" }, // UNRESOLVED dulu
                { reportedAt: "desc" }, // lalu yang terbaru
            ],
            take: 10,
            select: {
                issue: true,
        ...
        const waterIssues = issues.map((i) => {
            // judul ditambah identitas pelapor/pelanggan kalau ada
            let title = i.issue;
            if (i.pelanggan?.nama) {
                title += ` - ${i.pelanggan.nama} (${i.pelanggan.kode})`;
            } else if (i.reporterName) {
                title += ` - ${i.reporterName}`;
            }

            return {
                issue: title,
                // map enum → string yang dipakai di frontend
                status: i.status === "SOLVED" ? "solved" : "unresolved",
                date: i.reportedAt.toISOString().slice(0, 10),
            };
        });
        ...
        const trends = {
            ...
            totalTagihan: {
                value: Math.round(
                    pctChange(totalInclCarryNow, totalInclCarryPrev)
                ),
            ...
            pelanggan: {
                value: Math.round(
                    pctChange(totalPelangganNow, totalPelangganNow)
                ),
                isPositive: true,
            },
            payingRate: {
