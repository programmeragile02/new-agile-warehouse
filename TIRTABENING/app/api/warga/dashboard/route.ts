    ...
    try {
        ...
        if (latestPeriode) {
            ...
            if (tagihanNext) {
                ...
                currentUsage = {
                    period: `${BULAN[latestPeriode.bulan - 1]} ${
                        latestPeriode.tahun
                    }`,
                    ...
                    pemakaian:
                        cm?.pemakaianM3 ??
                        Math.max(
                            (cm?.meterAkhir ?? 0) - (cm?.meterAwal ?? 0),
                            0
                        ),
        ...
        for (const t of tagihanTahunIni) {
            ...
            if (periodeIdPrev) {
                const cmPrev = await q(
                    `get catatMeter ${prev.kode} pelanggan`,
                    () =>
                        prisma.catatMeter.findUnique({
                            where: {
                                periodeId_pelangganId: {
            ...
            if (!usageM3) {
                ...
                if (periodeIdP) {
                    const cmP = await q(
                        `get catatMeter ${t.periode} pelanggan`,
                        () =>
                            prisma.catatMeter.findUnique({
                                where: {
                                    periodeId_pelangganId: {
            ...
            const periodeCatatKode = periodeIdPrev ? prev.kode : t.periode;

            yearlyUsage.push({
                // keep 'month' for backward compat (label bulan tagihan)
                month: monthShort(billMonth),
                // expose periodeCatat explicitly (format "YYYY-MM")
                periodeCatat: periodeCatatKode,
                usage: usageM3, // pemakaian dari CATAT (P-1) atau fallback P
                bill: t.totalTagihan,
                status: ["paid", "lunas"].includes(
                    (t.statusBayar || "").toLowerCase()
                )
                    ? "paid"
                    : "unpaid",
            });
