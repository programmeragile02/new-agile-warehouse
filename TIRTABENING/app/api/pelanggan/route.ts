...
export async function POST(req: NextRequest) {
    ...
    try {
        ...
        const out = await prisma.$transaction(async (tx) => {
            ...
            if (!addons && companyId) {
                // fallback hit ke Warehouse sekali (tanpa cache lama) supaya tidak bergantung 100% ke cookie
                const resp = await fetchEntitlementsFromWarehouse(companyId);
                const extra = resp?.entitlements?.find(
                    (e) => (e.code || "").toLowerCase() === "extra.customers"
                );
                const num = Number(
                    extra?.value_number ?? extra?.value ?? extra?.value_string
                );
                addons = Number.isFinite(num)
