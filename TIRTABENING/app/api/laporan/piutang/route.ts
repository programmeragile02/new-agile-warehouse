...
function parseInfoTags(info?: string | null) {
    ...
    const closed = Array.from(
        info.matchAll(/\[CLOSED_BY:([0-9]{4}-\d{2})\]/gi)
    ).map((m) => m[1]);
    if (closed.length) meta.closedByPeriods = closed;
...
export async function GET(req: NextRequest) {
    ...
    try {
        ...
        const rowsAll: Row[] = items.map((t) => {
            ...
            const tagihanLaluNum = t.tagihanLalu || 0; // >0 = KURANG (hutang), <0 = SISA (kelebihan)
            const nett = tagihanBulanIni + tagihanLaluNum;
            const piutang = Math.max(nett - sumBayar, 0);

            const jt = new Date(t.tglJatuhTempo);
            const overdueDays = Math.max(
                Math.floor((+now - +jt) / 86400000),
                0
            );
            ...
            // ===== STATUS FIX =====
            // - Ada SISA dari bulan lalu? -> BELUM_LUNAS
            // - ATAU sudah BAYAR SEBAGIAN bulan ini (ada bayar tapi masih ada piutang)? -> BELUM_LUNAS
            // - Selain itu -> BELUM_BAYAR
            const status: Row["status"] =
                tagihanLaluNum < 0 || (sumBayar > 0 && piutang > 0)
                    ? "BELUM_LUNAS"
                    : "BELUM_BAYAR";
            ...
            return {
                ...
                zonaNama: t.pelanggan?.zona?.nama || null,
                tglJatuhTempo: t.tglJatuhTempo.toISOString(),
                totalTagihanBulanIni: tagihanBulanIni,
                tagihanLalu: tagihanLaluNum,
                totalTagihanNett: nett,
                totalBayar: sumBayar,
                piutang,
                overdueDays,
        ...
        // === Filter akhir: hanya yang masih ada piutang & tidak CLOSED_BY
        let filtered = rowsAll.filter(
            (r) =>
                r.piutang > 0 &&
                !(r.meta?.closedByPeriods && r.meta.closedByPeriods.length > 0)
        ...
        filtered.sort((a, b) => {
            for (const key of sortKeys) {
                switch (key) {
                    case "piutang_desc":
                        if (b.piutang !== a.piutang)
                            return b.piutang - a.piutang;
                        break;
                    case "piutang_asc":
                        if (a.piutang !== b.piutang)
                            return a.piutang - b.piutang;
                        break;
                    case "overdue_desc":
                        if (b.overdueDays !== a.overdueDays)
                            return b.overdueDays - a.overdueDays;
                    ...
                    case "nama_asc":
                        if (a.pelangganNama !== b.pelangganNama)
                            return a.pelangganNama.localeCompare(
                                b.pelangganNama
                            );
                        break;
                    case "nama_desc":
                        if (a.pelangganNama !== b.pelangganNama)
                            return b.pelangganNama.localeCompare(
                                a.pelangganNama
                            );
                        break;
                }
        ...
        const avgOverdue =
            filtered.length > 0
                ? Math.round(
                      filtered.reduce((s, r) => s + r.overdueDays, 0) /
                          filtered.length
                  )
                : 0;
