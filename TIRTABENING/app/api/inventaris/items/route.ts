...
/**
 * Gunakan tx yang dikirim (TransactionClient), JANGAN overwrite.
 * Asumsi KODE dipadded (WIDTH), jadi orderBy kode desc valid.
 */
async function generateNextKode(tx: Prisma.TransactionClient) {
    const last = await tx.item.findFirst({
        where: { deletedAt: null, kode: { startsWith: PREFIX } },
        orderBy: { kode: "desc" },
        select: { kode: true },
...
export async function GET() {
    ...
    try {
        ...
        // bantu debug di server log
        console.error("[GET /api/inventaris/items] Error:", e);
        return NextResponse.json(
...
export async function POST(req: NextRequest) {
    ...
    try {
        ...
        for (let i = 0; i < MAX_RETRY && !created; i++) {
            try {
                created = await prisma.$transaction(async (tx) => {
                    const kode = await generateNextKode(tx);
                    return tx.item.create({
                        data: {
                            kode,
                            nama: parsed.nama,
                ...
                lastErr = err;
                // P2002 = unique constraint violation (pastikan ada unique index di kolom `kode`)
                if (err?.code === "P2002") continue; // retry
                throw err; // error lain, hentikan
            }
        }

        if (!created) throw lastErr || new Error("Gagal membuat item");
        return NextResponse.json({ ok: true, item: created });
    } catch (e: any) {
        if (e?.name === "ZodError") {
            return NextResponse.json(
        ...
        console.error("[POST /api/inventaris/items] Error:", e);
        return NextResponse.json(
            { ok: false, message: e?.message ?? "Internal Server Error" },
            { status: 500 }
        );
