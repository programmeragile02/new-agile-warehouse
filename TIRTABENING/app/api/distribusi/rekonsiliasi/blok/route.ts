...
import { getPeriodeIdByKode, getBlokBalances } from "@/lib/rekon-balance";
export async function GET(req: NextRequest) {
  try {
    const { searchParams } = new URL(req.url);
    const kodePeriode = searchParams.get("periode"); // "YYYY-MM"
    const tandonId = searchParams.get("tandonId") || undefined;
    ...
    const rows = await getBlokBalances(periodeId, tandonId);
    return NextResponse.json({ ok: true, periode: kodePeriode, rows });
  } catch (e) {
    console.error("GET /api/distribusi/rekonsiliasi/blok", e);
    return NextResponse.json(
